//! SPI NOR Flash driver for STM32F4
//! Uses hardware SPI peripheral with DMA support for high-speed communication
//! with SPI NOR flash chips.
//!
//! STM32F4 advantages:
//! - Higher clock speeds (up to 42MHz on SPI1)
...
/// SPI NOR controller for STM32F4
///
/// Provides low-level access to SPI NOR flash chips using hardware SPI with DMA.
/// Supports 3-byte and 4-byte addressing modes.
///
/// STM32F4 specific features:
/// - DMA transfers for bulk read/write operations
/// - Higher SPI clock speeds (up to 42MHz)
...
impl<'d, SPI: embassy_stm32::spi::Instance> SpiNorController<'d, SPI> {
    ...
    ///
    /// # Arguments
    /// * `spi` - SPI peripheral instance configured for appropriate speed
    ...
    /// let mut nor = SpiNorController::new(spi, cs);
    /// ```
    pub fn new(spi: Spi<'d, SPI, Blocking>, cs: Output<'d>) -> Self {
        Self {
            spi,
            cs,
    ...
    /// Get current address byte count
    ...
    ///
    /// Returns [manufacturer_id, memory_type, capacity]
    ...
    ///
    /// SFDP provides standardized chip parameters for auto-detection.
    ///
    /// # Arguments
    ...
    /// For higher speeds, use fast_read().
    ///
    /// # Arguments
    ...
    pub fn read(&mut self, address: u32, buf: &mut [u8]) {
        ...
        if self.address_bytes == 4 {
            ...
            let _ =
                self.spi
                    .blocking_write(&[commands::READ, addr[0], addr[1], addr[2], addr[3]]);
        } else {
    ...
    ///
    /// Reads data at higher clock speeds (up to 42MHz on STM32F4).
